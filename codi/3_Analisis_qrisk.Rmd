...
dades %>% filter(DG.TMS=="Yes") %>% 
  select(idp,
         c(extreure.variables("QRISK",conductor_variables),extreure.variables("REGICOR",conductor_variables)) %>% unique()) %>% 
  head(10) %>% write.csv2("Ids_revisar.csv")

dades %>% select(edat,DG.TMS,DG.TMS_DIAG,FD.APSIC,FP.APSIC)

#
DG.APSIC=if_else(DG.TMS==0 & (FD.APSIC==1 | FP.APSIC==1 ) & edat<=65 & DG.DEMENCIA==0,1,0)


table(dades$DG.APSIC,dades$DG.TMS)

table(dades$DG.APSIC)

table(dades$DG.TMS)
table(dades$DG.TMS_DIAG)
...
descrTable(formu,dades,include.miss = F, show.n = T, show.all = T, show.p.overall = F,hide.no = T, extra.labels = c("","",""), 
           hide = "No") %>% 
  export2md(caption = "Descriptiva Baseline")
...
genera_output_HR<-function(x_event="EV.CVD",x_temps="EV.CVD_temps",dt=dt_temp) {
  ...
  cat(sjPlot::tab_model(model1,model2,show.aic=T,show.r2=T,title = label_outcome,dv.labels=R2_text)$knitr,"\n------\n") 
  ...
  cat(sjPlot::tab_model(model1,model2,show.aic=T,title = label_outcome,dv.labels=R2_text)$knitr,"\n------\n")
  ...
  print(fig$Home + ggtitle("ROC Curve (Males)"))
  
  cat("\n")  
  print (fig$Dona + ggtitle("ROC Curve (Females)"))
...
# purrr::map2(llistaevents[2],llista_temps[2],~genera_output_HR(.x,.y,dt_temp)) %>% invisible()
