...
funcions_lectura_dades<-function(mostra) {
  ...
  if (mostra==T) {
    ...
    LLEGIR.POBLACIO<<-function(n=Nmostra) {
      readRDS("./dades/mostra/poblacio_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.PROBLEMES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/problemes_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.FX.PRESCRITS<<-function(n=Nmostra) {
      readRDS("./dades/mostra/fx_prescrits_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.FX.FACTURATS<<-function(n=Nmostra) {
      readRDS("./dades/mostra/fx_facturats_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.TABAC<<-function(n=Nmostra) {
      readRDS("./dades/mostra/tabac_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.ANALITIQUES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/analitiques_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.CLINIQUES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/cliniques_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.VAR.GEOSANITARIES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/var_geosanitaries_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.SOCIOECONOMIQUES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/socioeconomiques_dt.rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.VISITES<<-function(n=Nmostra) {
      readRDS("./dades/mostra/visites_dt.Rds")%>% as_tibble() %>% head(n) }
    
    LLEGIR.VISITES_AGR<<-function(n=Nmostra) {
      readRDS("./dades/mostra/visitesagregades_dt.rds")%>% as_tibble() %>% head(n) }
